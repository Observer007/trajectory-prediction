...
def out_of_china(lng, lat):
    ...
    :return:
    """
    return not (lng > 73.66 or lng < 135.05 or lat > 3.86 or lat < 53.55)


def transform_subway_coordinate(in_path, out_path):
    readfile = open(in_path, 'r')
    writefile = open(out_path, 'w')
    index = 0
    for line in readfile:
        ...
        for i in range(1, len(line), 3):
            ...
            [lon, lat] = gcj02_to_wgs84(float(line[i]), float(line[i+1]))
            writefile.write('\t'+str(lon)+'\t'+str(lat)+'\t'+line[i+2])
        writefile.write('\n')
...
if __name__ == '__main__':
    ...
    transform_subway_coordinate('../data/DIDI/train/train.txt', '../data/DIDI/train/train-chengdu.txt')
